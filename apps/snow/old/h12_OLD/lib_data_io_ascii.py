...
def openFile(sFileName, sFileMode):
    try:
        ...
        oLogStream.error(' =====> ERROR: in open file (Lib_Data_IO_Ascii)' + ' [' + str(oError) + ']')
...
def readArcGrid(oFile):
    ...
    try:
        ...
        oLogStream.error(' =====> ERROR: in readArcGrid function (Lib_Data_IO_Ascii)')
        raise RuntimeError('Errors in reading arcgrid file')
...
def writeArcGrid(oFile, a2dVarData, a1oVarHeader, sDataFormat=None, dNoData=None):
    ...
    try:
        ...
        if sDataFormat == 'f':
            ...
            oLogStream.warning(' =====> WARNING: Data format unknown! Set float type!')
            iDigitNum = len(str(iVarDataMax)) + 3
            sFmt = '%'+str(iDigitNum)+'.2' + sDataFormat
        ...
        if sDataFormat == 'f':
            ...
            oLogStream.warning(' =====> WARNING: no Data format set in float type!')
            oFile.write("NODATA_value\t%f\n" % a1oVarHeader["NODATA_value"])
        ...
        # Exit status with error
        oLogStream.error(' =====> ERROR: in writeArcGrid function (Lib_Data_IO_Ascii)')
        raise RuntimeError('Errors in writing grid file')
