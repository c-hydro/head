...
def openFile(sFileName, sFileMode='r'):
    ...
    try:
        ...
        if sFileMode == 'r':
            ...
            oLogStream.warning(' -----> WARNING: open file in write/append mode not available (lib_data_io_grib)', 2, 1)
            return None

    except IOError as oError:
        oLogStream.error(' -----> ERROR: in open file (lib_data_io_grib)' + ' [' + str(oError) + ']')
...
def getFileMessage(oFile):
    ...
    try:
        ...
        if iFM == 0:
            a1iFM = None
            oLogStream.warning(' -----> WARNING: no message in GRIB file! Check your input file!')
        else:
...
def getVar2D_ALL(oFile, iIndex=None):
    ...
    try:
        ...
        oLogStream.warning(' -----> WARNING: in getting 2d variable function (lib_data_io_grib)')
...
def getVar3D_ALL(oFile, sVarComp=None):
    ...
    try:
            ...
            if (sVarComp == sVarName) or (sVarComp is None):
                ...
                if bVarWarn is False:
                    oLogStream.warning(
                        ...
                        '] is not in grib message (lib_data_io_grib)')
                    bVarWarn = True
        return oVarData
        ...
        # Exit status with warning
        oLogStream.warning(' -----> WARNING: in getting 3d variable function (lib_data_io_grib)')
...
def getVarGeo(oFile, iIndex=1):
    ...
    try:
        ...
        oLogStream.warning(' -----> WARNING: error in extracting file geographical data! Check input file!')
        a1oVarGeo = None
    ...
    for sVarKey in oVarHandle.keys():
        ...
        if sVarKey not in oVarKeyNA:
            ...
            try:
                ...
                oLogStream.warning(' -----> WARNING: file key ' +
                           sVarKey + ' not correctly retrieved Check input file!')
        else:
            pass
...
def getVarGeo_LAMI_2i(oFileHandle, iIdx=0):
    ...
    try:
        ...
        # Exit status with warning
        oLogStream.warning(' -----> WARNING: in getVarAttr_LAMI_2i an exception occurred (lib_data_io_grib)')
            ...
            if sVarComp is None:
                ...
                try:
                    ...
                    oLogStream.warning(
                        ...
                        sVarComp + '] is not in grib message (lib_data_io_grib)')
                    oFileMsg = None
...
def getVarTime_LAMI_2i(oFileHandle, oVarComp):
    ...
    try:
        ...
        # Exit status with warning
        oLogStream.warning(' -----> WARNING: in getVarTime_LAMI_2i an exception occurred (lib_data_io_grib)')
...
def getVar3D_LAMI_2i(oFileHandle, oVarComp=None):
    ...
    try:
        ...
        # Exit status with warning
        oLogStream.warning(' -----> WARNING: in getVar3D_LAMI_2i an exception occurred (lib_data_io_grib)')
