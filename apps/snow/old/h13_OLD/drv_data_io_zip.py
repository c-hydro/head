...
import lib_utils_apps_zip as Lib_File_Zip_Apps
...
class Drv_Data_Zip:
    ...
    def __init__(self, sFileName_IN, sZipMode='', sFileName_OUT=None, sZipType=None):
        ...
        if self.sFileName_IN.endswith('gz') or self.sZipType == 'gz':
            ...
            if sZipMode == 'z':
                oLogStream.error(' =====> ERROR: zip or unzip functions are not selected! Please check zip tag!')
                raise RuntimeError('Zip functions are not available')
            elif sZipMode == 'u':
                oLogStream.warning(' =====> WARNING: zip or unzip functions are not selected! Please check zip tag!')
    ...
    def __setFileNameIN(self):
        ...
        if self.sFileName_IN:
            ...
            oLogStream.error(' =====> ERROR: input filename is not defined! Please check driver argument(s)!')
...
class BZ2:
    ...
    def setFileLibZip(self):
        ...
        import lib_data_zip_bz2 as oFileLibrary
...
class GZip:
    ...
    def setFileLibZip(self):
        ...
        import lib_data_zip_gzip as oFileLibrary
        self.oFileLibrary = oFileLibrary
