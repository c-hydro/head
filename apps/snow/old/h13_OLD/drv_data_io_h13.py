...
from numpy import where, reshape, unique

from lib_analysis_filtering import defineLookUpTable
from lib_analysis_interpolation_grid import interpGridData, interpGridIndex

from lib_utils_apps_data import updateDictStructure
from lib_utils_apps_file import handleFileData, selectFileDriver, zipFileData
from lib_utils_file_workspace import savePickle, restorePickle

from lib_default_args import sZipExt as sZipExt_Default
from lib_default_args import sTimeFormat as sTimeFormat_Default
from lib_default_args import sTimeCalendar as sTimeCalendar_Default
from lib_default_args import sTimeUnits as sTimeUnits_Default
from lib_default_conventions import oVarConventions as oVarConventions_Default
from lib_default_conventions import oFileConventions as oFileConventions_Default
    ...
    def computeDataProduct(self, oDataGeo=None):
        ...
        for sVarKey, oVarData in oVarWS.items():
            ...
            if oVarWS[sVarKey] is not None:
                ...
                if ('values' in oVarData) and ('attributes' in oVarData):
                    ...
                    if ('flag_masks' in oVarAttrs) and ('flag_values' in oVarAttrs) and ('flag_meanings' in oVarAttrs):
                        ...
                        for sTableKey, oTableValue in oVarTable.items():
                            ...
                            if oTableValue['in']['min'] is not None:
                                dTableIN_MIN = float(oTableValue['in']['min'])
                            else:
                                dTableIN_MIN = None
                            if oTableValue['in']['max'] is not None:
                    ...
                    # plt.figure()
                    # plt.imshow(a2dVarValue_INTERP); plt.colorbar(); plt.clim(0, 100)
                    # plt.figure()
                    # plt.imshow(a2dVarValue_FILTER); plt.colorbar(); plt.clim(0, 100)
                    # plt.show()
                    # -------------------------------------------------------------------------------------
                    ...
                    oLogStream.warning(' ---> Get variable: ' + sVarKey +
                               ' ... FAILED! Values and/or Attributes field(s) is/are not defined!')
                ...
                # -------------------------------------------------------------------------------------
                # Exit variable key not in workspace
                oLogStream.warning(' ---> Compute variable: ' + sVarKey +
                           ' ... FAILED! Variable data is None!')
                oVarWS[sVarKey] = None
                # -------------------------------------------------------------------------------------
        ...
        for sVarKey, oVarDef in self.oVarDef.items():
            ...
            if self.oVarData[sVarKey] is not None:
                ...
                if sVarFile in self.oVarFile:
                    ...
                    # -------------------------------------------------------------------------------------
                    # Exit variable key not in workspace
                    oLogStream.warning(' ---> Save file: ' + sVarFileName +
                               ' ... FAILED! File is not in declared workspace!')
                    # Exit without saving variable
                    oLogStream.warning(' ---> Save variable: ' + sVarKey + ' ... FAILED')
                    # -------------------------------------------------------------------------------------
                ...
                # -------------------------------------------------------------------------------------
                # Exit without saving variable
                oLogStream.warning(' ---> Save variable: ' + sVarKey + ' ... FAILED! Variable data is None!')
                # -------------------------------------------------------------------------------------
        ...
        for sVarKey, sVarFile in self.oVarFile.items():
            ...
            if sVarKey in self.oVarData.keys():
                ...
                if bFileOpen is True:
                    ...
                    # -------------------------------------------------------------------------------------
                    # Exit variable key not in workspace
                    oLogStream.warning(' ---> Get variable: ' + sVarKey + ' ... FAILED! File not correctly open!')
                    oVarWS[sVarKey] = None
                    # -------------------------------------------------------------------------------------
                ...
                # Exit variable key not in workspace
                oLogStream.warning(' ---> Get variable: ' + sVarKey +
                           ' ... FAILED! Variable is not in declared workspace!')
                oVarWS[sVarKey] = None
                # -------------------------------------------------------------------------------------
