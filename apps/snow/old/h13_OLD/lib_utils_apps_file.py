...
def importFileDict(sFileName, DataName='Data'):
    ...
    if exists(sFileName):
        try:
            ...
            oLogStream.warning(' =====> WARNING: read file ' + sFileName + ' FAILED!')
            oFileDict = None
            bFileData = False
    else:
        oLogStream.warning(' =====> WARNING: file ' + sFileName + ' NOT FOUND!')
        oFileDict = None
        bFileData = False
...
def selectFileDriver(sFileName, sZipExt=None, sFileMode=None):
    ...
    if (not exists(sFileUnzip)) and (not exists(sFileZip)):
        ...
        try:
            ...
            # Exception
            oLogStream.error(' =====> ERROR: driver selection to open file ' +
                             sFileName + ' FAILED! Errors in opening file in write mode!')
        ...
        try:
            ...
            # Exception
            oLogStream.error(' =====> ERROR: driver selection to unzip or open file ' +
                             sFileZip + ' FAILED! Errors in unzipping or opening file in append mode!')
        ...
        try:
            ...
            # Exception
            oLogStream.error(' =====> ERROR: driver selection open file ' +
                             sFileUnzip + ' FAILED! Errors in opening file in append mode!')
            raise RuntimeError('Open file failed')
        # -------------------------------------------------------------------------------------
        ...
        # Exception
        oLogStream.error(' =====> ERROR: driver selection open file ' +
                         sFileUnzip + ' FAILED! Errors in selecting driver to open file!')
        raise RuntimeError('Open file failed')
        # -------------------------------------------------------------------------------------
...
def handleFileData(sFileName, sPathTemp="", bFileTemp=True):
    ...
    if exists(sFileName):
        ...
        if bZipType_Destination is True:
            ...
            try:
                ...
                oLogStream.warning(' =====> WARNING: handle file ' + sFileName +
                                   ' FAILED! Errors in unzipping or opening file!')
                oFileDriver = None
                oFileHandle = None
                bFileOpen = False
                # -------------------------------------------------------------------------------------
            ...
            try:
                ...
                oLogStream.warning(' =====> WARNING: handle file ' + sFileName + ' FAILED! Error in opening file!')
                oFileDriver = None
                oFileHandle = None
                bFileOpen = False
                # -------------------------------------------------------------------------------------
        ...
        oLogStream.warning(' =====> WARNING: handle file ' + sFileName + ' FAILED! File not found!')
        oFileDriver = None
        oFileHandle = None
        bFileOpen = False
        # -------------------------------------------------------------------------------------
