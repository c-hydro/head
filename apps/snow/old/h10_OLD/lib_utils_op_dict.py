...
from lib_utils_op_list import convertList2Dict
...
def prepareDictKey(ob_keys, sep_keys=''):
    try:
        if isinstance(ob_keys, str):
            ...
            oLogStream.error(' =====> ERROR: keys format unknown!')
            raise RuntimeError('Dictionary keys format unknown')
...
def lookupDictKey(dic, key, *keys):
    try:
        ...
        oLogStream.warning(' =====> WARNING: impossible to get dictionary value using selected keys!')
        return None
# -------------------------------------------------------------------------------------
...
def removeDictKey(d, keys):
    ...
    if isinstance(keys, list):
        ...
        oLogStream.warning(' =====> WARNING: keys values must be included in a list!')
        return d
# -------------------------------------------------------------------------------------
...
def checkDictKeys(a1oVarCheck={'VarDefault': False}, sVarTag='variable(s)'):
    ...
    if a1oVarCheck.__len__() != iVarF and iVarF > 0:
        ...
        oLogStream.warning(' =====> WARNING: ' + sVarTag + ' ' + a1sVarKeyNF + ' not defined in given dictionary!')
    elif a1oVarCheck.__len__() == iVarF and iVarF > 0:
        # Exit message if all variable(s) are not available
        oLogStream.warning(' =====> WARNING: all ' + sVarTag + ' not defined in given dictionary!')
    else:
        pass
