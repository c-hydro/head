...
def openFile(sFileName, sFileMode, sFileFormat='NETCDF4'):
    ...
    try:
        ...
        oLogStream.warning(' =====> WARNING: NetCDF module import from scipy (usually imported from netcdf-python)!')
    ...
    try:
        ...
        oLogStream.error(' =====> ERROR: in open file (lib_data_io_netcdf)' + ' [' + str(oError) + ']')
...
def getVarAttrs(oFileData, sVarName, sVarEncode=None):
    ...
    for sAttrName_IN in oFileData.variables[sVarName].ncattrs():
        ...
        if sVarEncode is None:
            ...
            oLogStream.warning(' =====> WARNING: in reading variable attribute ' + sAttrName_OUT + '. '
                       + 'Attribute encoding unknown or not correctly defined! (lib_data_io_netcdf)')
        ...
        if isinstance(oAttrValue_RAW, str):
            if sVarEncode is None:
                ...
                oLogStream.warning(' =====> WARNING: in reading variable attribute' + sAttrName_OUT + '. ' +
                                   'Casting value unsing a default encoding! (lib_data_io_netcdf)')
                oAttrValue_OUT = oAttrValue_RAW
            ...
            oLogStream.warning(
                ...
                + 'Attribute type not defined! (lib_data_io_netcdf)')
            oAttrValue_OUT = None
...
def getFileAttrs(oFileData, sVarEncode=None):
    ...
    for sAttrName in oAttrData:
        ...
        if sVarEncode is None:
            ...
            oLogStream.warning(' =====> WARNING: in reading variable attribute ' + sAttrName + '. ' +
                               'Attribute encoding unknown or not correctly defined! (lib_data_io_netcdf)')
        ...
        if sVarEncode is None:
            ...
            oLogStream.warning(' =====> WARNING: in storing variable attribute ' + sAttrName + '. ' +
                               'Attribute encoding unknown or not correctly defined! (lib_data_io_netcdf)')
...
def get1DVar(oFileData, sVarName):
    ...
    try:
        ...
        a1dVarName_OUT = None
        oLogStream.error(' =====> ERROR: in reading variable 1d. Check variable name! (lib_data_io_netcdf)')
        raise RuntimeError('Read variable failed')
...
def get2DVar(oFileData, sVarName, bSetAutoMask=True):
    ...
    if not bSetAutoMask:
        oLogStream.warning(' =====> WARNING: auto_mask is set to false! (lib_data_io_netcdf)')
    ...
    try:
        ...
        a2dVarName_OUT = None
        oLogStream.error(' =====> ERROR: in reading variable 2d. Check variable name! (lib_data_io_netcdf)')
        raise RuntimeError('Read variable failed')
...
def write3DVar(oFileData, sVarName, a3dVarDataXYT, oVarAttr, sVarFormat, sVarDimT=None, sVarDimY=None, sVarDimX=None):
    ...
    if not sVarDimY or not sVarDimX or not sVarDimT:
        oLogStream.error(
            ...
            + 'Dimensions are not correctly defined! (lib_data_io_netcdf)')
        raise RuntimeError('Save variable failed')
    else:
        pass
    ...
    if a3dVarDataXYT is not None:
        ...
        if oVarAttr:
            ...
            oLogStream.warning(
                ...
                + 'Attributes are null but variable saved correctly! (lib_data_io_netcdf)')
        ...
        oLogStream.warning(
            ' =====> WARNING: in saving file 3d variable ' + sVarName + '. '
            + 'Variable is null and not saved! (lib_data_io_netcdf)')
