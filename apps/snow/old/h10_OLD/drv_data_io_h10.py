        ...
        for sVarKey, oVarData in oVarWS.items():
            ...
            if oVarWS[sVarKey] is not None:
                ...
                if ('values' in oVarData) and ('attributes' in oVarData):
                    ...
                    # plt.figure()
                    # plt.imshow(a2dVarValue_INTERP); plt.colorbar(); plt.clim(0, 2)
                    # plt.figure()
                    # plt.imshow(a2dVarValue_FILTER); plt.colorbar(); plt.clim(0, 2)
                    # plt.show()
                    # -------------------------------------------------------------------------------------
                ...
                # -------------------------------------------------------------------------------------
                # Exit variable key not in workspace
                oLogStream.warning(' ---> Compute variable: ' + sVarKey + ' ... FAILED! Variable data is None!')
                oVarWS[sVarKey] = None
                # -------------------------------------------------------------------------------------
        ...
        for sVarKey, oVarDef in self.oVarDef.items():
            ...
            if sVarFile in self.oVarFile:
                ...
                if exists(sVarFileName):
                    ...
                    if bFileOpen is True:
                        ...
                        for sFileVar, oFileData in oFileDict.items():
                            ...
                            if sFileVar == sVarName:
                                ...
                                oLogStream.info(' ---> Get file variable: ' + sFileVar + ' ... SKIPPED!')
                                # -------------------------------------------------------------------------------------
                    ...
                    # -------------------------------------------------------------------------------------
                    # Exit variable key not in workspace
                    oLogStream.warning(' ---> Get file: ' + sVarFileName + ' ... FAILED! File not found!')
                    oVarWS[sVarKey] = None
                    # -------------------------------------------------------------------------------------
                ...
                oLogStream.warning(' ---> Get file: ' + sVarFileName + ' ... FAILED! File is not in declared workspace!')
                oVarWS[sVarKey] = None
                # -------------------------------------------------------------------------------------
