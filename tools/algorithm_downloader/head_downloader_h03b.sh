...
# ----------------------------------------------------------------------------------------
# Script information
script_name='HEAD DOWNLOADER - HSAF PRODUCT PRECIPITATION H03B'
script_version="2.0.1"
script_date='2019/04/17'

# Script argument(s)
data_folder_raw="/home/hsaf/hsaf_datasets/dynamic/source/h03b/%YYYY/%MM/%DD/"
days=5
proxy=""
...
ftprem`
	...
	for file_name_zip in ${ftp_file_list}; do
	    ...
	    echo " ====> CREATE NC FILE: ${file_name_nc} ..."
	    if ! [ -e ${file_path_gzip} ]; then
	        ...
	        if ! [ -e ${file_path_grib} ]; then
	            if gunzip -k $file_path_zip > /dev/null 2>&1; then
                    ...
                    echo " FAILED! Error in command execution! [UNZIP]"
                    create_nc=false
                    exit 1
                fi
		    else
	        ...
	        if ! [ -e ${file_path_tmp_step1} ]; then
	            ...
	            if ${wgrib2_exec} ${file_path_grib} -netcdf ${file_path_tmp_step1} > /dev/null 2>&1; then
                    ...
                    echo " FAILED! Error in command execution! [CONVERT FROM GRIB TO NC]"
                    create_nc=false
                    exit 2
                fi
	        ...
	        if ! [ -e ${file_path_tmp_step2} ]; then
	            ...
	            if ${ncks_exec} -C -x -v ${list_var_drop} ${file_path_tmp_step1} ${file_path_tmp_step2} > /dev/null 2>&1; then
                    ...
                    echo " ... REDUCING VARIABLE ... FAILED! Error in command execution! [REDUCE VARS]"
                    create_nc=false
                    exit 3
                fi
                
                echo "COMMAND RENAMING VAR1: " ${ncrename_exec} -h -O -v ${var_rename_1} ${file_path_tmp_step2}
                if ${ncrename_exec} -h -O -v ${var_rename_1} ${file_path_tmp_step2} > /dev/null 2>&1; then
                    echo " ... RENAMING VAR-1 ... DONE!"
                else
                    echo " ... RENAMING VAR-1 ... FAILED! Error in renaming variable 1! [RENAME VAR1]"
                    create_nc=false
                fi
                
                echo "COMMAND RENAMING VAR1: " ${ncrename_exec} -h -O -v ${var_rename_2} ${file_path_tmp_step2}
                if ${ncrename_exec} -h -O -v ${var_rename_2} ${file_path_tmp_step2} > /dev/null 2>&1; then
                    echo " ... RENAMING VAR-2 ... DONE!"
                else
                    echo " ... RENAMING VAR-2 ... FAILED! Error in renaming variable 2! [RENAME VAR2]"
                    create_nc=false
                fi
	        ...
	        if ! [ -e ${file_path_nc} ]; then
	            ...
	            if ${cdo_exec} sellonlatbox,${domain_bb} ${file_path_tmp_step2} ${file_path_nc} > /dev/null 2>&1; then
                    ...
                    echo " FAILED! Error in command execution! [SELECT FILE OVER DOMAIN]"
                    create_nc=false
                fi
	        ...
	        if [ "$create_nc" = true ] ; then
                ...
                echo " FAILED! Errors occurred in nc file creation! [CHECK AND COMPRESS FILE NC]"
        	    if [ -e ${file_path_nc} ]; then
	                rm ${file_path_nc} 
                fi
                create_nc=false
