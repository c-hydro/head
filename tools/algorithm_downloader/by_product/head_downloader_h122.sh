...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h122.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_SM/ascat/nrt/h122/%YYYY/%MM/%DD/%HH/"

# Remote folder template
export FTP_FOLDER_RAW="/products/h122/h122_cur_mon_nc/"
...
LOG_DIR="/share/LOGS/hsaf_h122"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h122_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H122 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H122 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
