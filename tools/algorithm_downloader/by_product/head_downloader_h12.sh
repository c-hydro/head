...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h12.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_SNOW/nrt/h12/%YYYY/%MM/%DD/"

# Remote FTP folder
export FTP_FOLDER="/products/h12/h12_cur_mon_data"
...
LOG_DIR="/share/LOGS/hsaf_h12"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h12_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H12 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H12 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
