...
# ----------------------------------------------------------------------------------------
# Script information
SCRIPT_NAME = "HEAD DOWNLOADER - HSAF PRODUCT PRECIPITATION H60"
SCRIPT_VERSION = "3.3.1"
SCRIPT_DATE = "2025/11/18"

# === User settings (H60/H60B) ==========================================================
...
# Product prefixes to download (supports multiple, e.g. h60 and h60b)
# Comma-separated environment variable, default "h60"
PRODUCT_PREFIXES = [
    p.strip() for p in os.environ.get("PRODUCT_PREFIXES", "h60").split(",") if p.strip()
]

# Local path pattern (no %HH)
LOCAL_FOLDER_RAW = os.environ.get(
    "LOCAL_FOLDER_RAW",
    "/share/HSAF_PRECIPITATION/nrt/h60/%YYYY/%MM/%DD/",
)

# Remote folder template (H60/H60B)
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description=f"{SCRIPT_NAME} (Python, single-session lftp, H60/H60B with quarters)"
    )
    parser.add_argument(
    ...
    print(f" ==> Product prefixes: {', '.join(PRODUCT_PREFIXES)}")
    if plan_mode:
    ...
    for day_offset in range(DAYS + 1):
        ...
        for hour in range(count_start, count_end - 1, -1):
            ...
            # For realtime current day + current hour, apply SAFETY_LAG_MIN to quarters
            ...
            if (
                ...
                adj = now_min - SAFETY_LAG_MIN
                if adj < 0:
                    print(
                        f"  [INFO] No completed quarters yet (safety lag {SAFETY_LAG_MIN}m). "
                        f"Skipping this hour."
                    )
            ...
            print(f"  [INFO] Quarters: {', '.join(quarters) if quarters else '(none)'}")
            ...
            # Expected filenames over DAYS (date x hour x quarter x prefix)
            for q in quarters:
                for prefix in PRODUCT_PREFIXES:
                    ftp_file = f"{prefix}_{date_step_str2}_{hour_get}{q}_fdk.nc.gz"
