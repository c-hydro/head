...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h64.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_PRECIPITATION/nrt/h64/%YYYY/%MM/%DD/"

# Remote FTP folder
export FTP_FOLDER="/products/h64/h64_cur_mon_data"
...
LOG_DIR="/share/LOGS/hsaf_h64"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h64_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H64 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H64 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
