...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h26.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_SM/ecmwf/nrt/h26/%YYYY/%MM/%DD/"

# Remote FTP folder
export FTP_FOLDER="/products/h26/h26_cur_mon_nc/"
...
LOG_DIR="/share/LOGS/hsaf_h26"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h26_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H26 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H26 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
