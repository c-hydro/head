...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h60.py"
...
# (No %HH here; hours are handled inside the Python script)
export LOCAL_FOLDER_RAW="/share/HSAF_PRECIPITATION/nrt/h60/%YYYY/%MM/%DD/"

# Remote folder template
# - realtime: current-month folder without date/hour tokens
# - history : /hsaf_archive/h60/%YYYY/%MM/%DD/%HH/ (handled inside Python by SCRIPT_MODE)
export FTP_FOLDER_RAW="/products/h60/h60_cur_mon_data/"
...
LOG_DIR="/share/LOGS/hsaf_h60"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h60_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H60 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H60 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
