...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h14.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_SM/ecmwf/nrt/h14/%YYYY/%MM/%DD/"

# Remote FTP folder
export FTP_FOLDER="/products/h14/h14_cur_mon_grib"
...
LOG_DIR="/share/LOGS/hsaf_h14"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h14_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H14 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H14 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
