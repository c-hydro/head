...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h10.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_SNOW/nrt/h10/%YYYY/%MM/%DD/"

# Remote FTP folder
export FTP_FOLDER="/products/h10/h10_cur_mon_data"
...
LOG_DIR="/share/LOGS/hsaf_h10"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h10_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H10 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H10 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
