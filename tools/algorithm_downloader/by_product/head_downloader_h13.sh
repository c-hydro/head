...
PY_SCRIPT="/root/projects/hsaf/algorithm_downloader/head_downloader_h13.py"
...
export LOCAL_FOLDER_RAW="/share/HSAF_SNOW/nrt/h13/%YYYY/%MM/%DD/"

# Remote FTP folder
export FTP_FOLDER="/products/h13/h13_cur_mon_data"
...
LOG_DIR="/share/LOGS/hsaf_h13"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/hsaf_h13_$(date +%Y%m%d).log"
...
{
  echo "============================================================"
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] START H13 PYTHON DOWNLOADER"
  ...
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] END H13 PYTHON DOWNLOADER (rc=${rc})"
  echo
} 2>&1 | tee -a "$LOG_FILE"
